...
# endif /* _MSC_VER */
    ...
    if (sys_externalschedlib)
        {
	post ("using external scheduler lib : %s.dll\n",sys_externalschedlibname);
        ...
        sys_stderr = freopen("pdstderr.txt", "wt", stderr);
        sprintf(filename, "%s.dll", sys_externalschedlibname);
        sys_bashfilename(filename, filename);
        ntdll = LoadLibrary(filename);
        if (!ntdll)
        {
              post("/n%s: couldn't load external scheduler lib ", filename);
              return (0);
        }
	else post ("ext scheduler %s loaded\n",filename);
        externalmainfunc = (t_externalschedlibmain)GetProcAddress(ntdll,
                                                                  "_main");
	post("externalmainfunc is loaded");
        // return 0;
...
"-prefs           -- fload preferences on startup (true by default)\n",
